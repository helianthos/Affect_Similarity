...
The normality and heteroscedasticity concern does not come from the higher concentrartion of beeps with love = 100, but from the boundary itself. Even after removing the dyads with the most ceiling-dominated participants, the remaining participants still cannot score above 100. So whenever the model predicts a fitted value near the upper range, the residuals are necessarily truncated on the positive side. A participant with a fitted value of 85 for example can only have a residual of at most +15, whereas they can have a residual of -85. This asymmetry is structural.\
\
...
plot(p) + ggtitle("Within Person Mediation Diagram")
...
-   a-path: $\theta_2 = 0.708$ ($p < .001$) (actual similarity → perceived similarity)

-   b-path: $ϕ_2=0.859$ $(p<.001)$ (perceived similarity → love, controlling actual)

-   c-path: $\beta_2=0.214$, $p = .43$ , 95% CI $[−0.314,0.742]$, ns. (actual similarity → love, total)

-   c'-path: $\phi_4 = −0.355$ $p = .249$ , ns, (actual similarity → love, direct)

-   **Indirect effect** = $θ_2ϕ_2= 0.708 × 0.859 ≈ 0.608$
...
plot(p) + ggtitle("Between Person Mediation Diagram")
...
n_sim <- 200  # number of Monte Carlo draws (20k is standard; use 50k for publication)
...
cprime_w   <- fixef(h21_outcome)["PA_similarity_act_w"]
cprime_b   <- fixef(h21_outcome)["PA_similarity_act_b"]
...
se_c_w <- sqrt(vcov(h11)["PA_similarity_act_w", "PA_similarity_act_w"])
se_c_b <- sqrt(vcov(h11)["PA_similarity_act_b", "PA_similarity_act_b"])
...
# Direct and total effects with CIs
# These come directly from the models (no simulation needed — single params)

cat("\n============================================================")
cat("\nDIRECT EFFECTS (c'-paths from outcome model)\n")
summarise_path(cprime_w, se_cprime_w, "Within direct effect (c'_w)")
summarise_path(cprime_b, se_cprime_b, "Between direct effect (c'_b)")

cat("\n============================================================")
cat("\nTOTAL EFFECTS (c-paths from base model)\n")
summarise_path(c_w, se_c_w, "Within total effect (c_w)")
summarise_path(c_b, se_c_b, "Between total effect (c_b)")

# ── STEP 9: Proportion mediated ────────────────────────────────────────────
# Indirect / Total; only meaningful when total effect is significant
# Monte Carlo distribution for proportion mediated
...
cat("\nPROPORTION MEDIATED\n")
cat(sprintf(
  "\nWithin-level: %.1f%% (95%% MC CI: [%.1f%%, %.1f%%])\n",
  result_w$point / c_w * 100,
  quantile(prop_w_sim, 0.025) * 100,
  quantile(prop_w_sim, 0.975) * 100
))
cat(sprintf(
  "Between-level: %.1f%% (95%% MC CI: [%.1f%%, %.1f%%])\n",
  result_b$point / c_b * 100,
  quantile(prop_b_sim, 0.025) * 100,
  quantile(prop_b_sim, 0.975) * 100
))
...
cat(strrep("-", 65), "\n")
cat(sprintf("%-35s %8.4f %20s\n", "a_w (act_w -> perc_w)",    a_w,      sprintf("[%.4f, %.4f]", a_w - 1.96*se_a_w, a_w + 1.96*se_a_w)))
cat(sprintf("%-35s %8.4f %20s\n", "a_b (act_b -> perc_b)",    a_b,      sprintf("[%.4f, %.4f]", a_b - 1.96*se_a_b, a_b + 1.96*se_a_b)))
cat(sprintf("%-35s %8.4f %20s\n", "b_w (perc_w -> love)",     b_w,      sprintf("[%.4f, %.4f]", b_w - 1.96*se_b_w, b_w + 1.96*se_b_w)))
cat(sprintf("%-35s %8.4f %20s\n", "b_b (perc_b -> love)",     b_b,      sprintf("[%.4f, %.4f]", b_b - 1.96*se_b_b, b_b + 1.96*se_b_b)))
cat(sprintf("%-35s %8.4f %20s\n", "c_w  (total within)",      c_w,      sprintf("[%.4f, %.4f]", c_w - 1.96*se_c_w, c_w + 1.96*se_c_w)))
cat(sprintf("%-35s %8.4f %20s\n", "c_b  (total between)",     c_b,      sprintf("[%.4f, %.4f]", c_b - 1.96*se_c_b, c_b + 1.96*se_c_b)))
cat(sprintf("%-35s %8.4f %20s\n", "c'_w (direct within)",     cprime_w, sprintf("[%.4f, %.4f]", cprime_w - 1.96*se_cprime_w, cprime_w + 1.96*se_cprime_w)))
cat(sprintf("%-35s %8.4f %20s\n", "c'_b (direct between)",    cprime_b, sprintf("[%.4f, %.4f]", cprime_b - 1.96*se_cprime_b, cprime_b + 1.96*se_cprime_b)))
cat(sprintf("%-35s %8.4f %20s\n", "Indirect_w (MC)",          result_w$point, sprintf("[%.4f, %.4f]", result_w$ci_low, result_w$ci_hi)))
cat(sprintf("%-35s %8.4f %20s\n", "Indirect_b (MC)",          result_b$point, sprintf("[%.4f, %.4f]", result_b$ci_low, result_b$ci_hi)))
```

At the **within-person level**, the indirect effect of actual PA similarity on love via perceived PA similarity was significant and substantial (indirect effect = 0.084, 95% MC CI \[0.078, 0.092\]). This corresponded to 74.5% (95% MC CI \[65.4%, 81.3%\]) of the total within-level effect (c_w = 0.113, CI \[0.104, 0.122\]), with a small but significant direct effect remaining (c'\_w = 0.032, CI \[0.020, 0.043\]). These results indicate **partial mediation** at the within-person level: in moments when partners are more similarly positive than usual, the actor perceives greater similarity with their partner, and this heightened perceived similarity in turn predicts greater momentary love. A small residual direct effect suggests an additional pathway from actual to perceived similarity that operates outside conscious perceptual awareness.

At the **between-dyad level**, the indirect effect was also significant (indirect effect = 0.609, 95% MC CI \[0.234, 0.875\]), indicating that dyads higher in average actual PA similarity perceived themselves as more similar, which in turn predicted higher average love. However, the between-level total effect was not significant (c_b = 0.214, CI \[−0.308, 0.736\]), and the direct effect was negative though also non-significant (c'\_b = −0.355, CI \[−0.955, 0.245\]). This pattern of a significant positive indirect effect alongside a near-zero total effect reflects an **inconsistent mediation** structure (MacKinnon et al., 2000): the positive mediated pathway is partially offset by a negative direct pathway, the two approximately cancelling at the total-effect level. Given this, the proportion mediated is not interpretable at the between level. The significant indirect effect nonetheless supports the conclusion that perceived similarity is a meaningful carrier of between-dyad differences in actual similarity's association with love, while the suppression pattern at the direct path level likely reflects the same directional asymmetry identified in the elevation sensitivity analysis — namely that actual PA similarity conflates elevation and directionality effects that relate differently to love once the perceptual pathway is controlled.

...
At the between-dyad level, the indirect effect was also significant (indirect effect $= 0.609$, 95% MC CI $[0.234, 0.875]$), indicating that dyads higher in average actual PA similarity perceived themselves as more similar, which in turn predicted higher average love. However, the between-level total effect was not significant ($\beta_2 = 0.214$, CI $[-0.308, 0.736]$), and the direct effect was negative though also non-significant ($\phi_4 = -0.355$, CI $[-0.955, 0.245]$). This pattern of a significant positive indirect effect alongside a near-zero total effect reflects an inconsistent mediation structure (@mackinnonEquivalenceMediationConfounding2000): the positive mediated pathway is partially offset by a negative direct pathway, the two approximately cancelling at the total-effect level. Given this, the proportion mediated is not interpretable at the between level. The significant indirect effect nonetheless supports the conclusion that perceived similarity is a meaningful carrier of between-dyad differences in actual similarity's association with love, while the suppression pattern at the direct path level likely reflects the same directional asymmetry identified in the elevation sensitivity analysis — namely that actual PA similarity conflates elevation and directionality effects that relate differently to love once the perceptual pathway is controlled.
...
-   a-path: $\theta_2 = 0.676$ ($p < .001$) (actual similarity → perceived similarity)

-   b-path: $ϕ_2=0.893$ $(p=.001)$ (perceived similarity → love, controlling actual)

-   c-path: $\beta_2=0.621$, $p = .005$ , 95% CI $[0.197, 1.045]$ (actual similarity → love, total)

-   c'-path: $\phi_4 = 0.010$ $p = .972$ , ns (actual similarity → love, direct)
...
with $R_{mpd}$ the momentary perceived partner responsiveness (person-mean centered). We focus on within-person moderation by person-mean centering 𝑅𝑚𝑝𝑑; we do not model between-person differences in typical responsiveness as a moderator.
...
"*Higher momentary perceived responsiveness is expected to **amplify** the positive association between actual PA similarity and love.*"

To have responsiveness as a buffer, we expect $\beta_4 >0$

```{r PA_responsiveness_moderation}
#| output-fold: true
#| output-summary: "Model results"
...
tbl_h31 <- data.frame(
  ...
  `Responsiveness value` = sprintf("%.2f", slopes_h31_df$cperc_resp),
  `Simple slope`         = sprintf("%.4f", slopes_h31_df$PA_similarity_act_w.trend),
...
$$
L_{mpd}​=b_{0,pd}​+ϕ_1​S_{mpd}^{perc,within}​+ϕ_2\bar{​S}_d^{perc,between}​+ϕ_3​S_{mpd}^{act,within}​+ϕ_4\bar{​S}_d^{act,between}​+ϕ_5​_{Rmpd}​+\newlineϕ_6​(S_{mpd}^{perc,within}​×R_{mpd}​)+ϕ_7​(S_{mpd}^{act,within}​×R_{mpd}​)+ε_{mpd}
$$

```{r h31_including_perceived similarity}
#| output-fold: true
#| output-summary: "Model results"
...
kable(tbl_extended,
  align    = c("l", "c", "c", "c", "c", "c", "c"),
  booktabs = TRUE,
  caption  = "Simple slopes of actual and perceived PA similarity on love at low, average, and high perceived responsiveness (h31_extended)"
) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover"),
    full_width = FALSE,
...
"*Higher momentary perceived partner responsiveness is expected to **attenuate** the negative association between actual NA similarity and love.*"

To have responsiveness as an amplifier, we expect $\beta_4 >0$, making the NA similarity-love slope ($β_1=-0.152$, $p<.001$, 95% CI: $[-0.167, -0.136]$ from model `h12`) less negative at higher perceived partner responsiveness.

```{r NA_responsiveness_moderation}
#| output-fold: true
#| output-summary: "Model results"

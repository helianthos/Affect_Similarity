...
function init() {
  window.addEventListener("pageshow", (_event) => {
    ...
    function toggleAll(selectedGroup, selectorsToSync2) {
      for (const [thisGroup, tabs] of Object.entries(selectorsToSync2)) {
    ...
    function findSelectorsToSyncByLanguage() {
      ...
      for (const item of tabs) {
        ...
        const selectorsToSync2 = result[group];
        const value = item.innerHTML;
        if (!selectorsToSync2[value]) {
          selectorsToSync2[value] = [];
        }
        selectorsToSync2[value].push(item);
      }
      return result;
    }
    function setupSelectorSync() {
      const selectorsToSync2 = findSelectorsToSyncByLanguage();
      Object.entries(selectorsToSync2).forEach(([group, tabSetsByValue]) => {
        Object.entries(tabSetsByValue).forEach(([value, items]) => {
          items.forEach((item) => {
            item.addEventListener("click", (_event2) => {
              setTabState(group, value);
              toggleAll(value, selectorsToSync2[group]);
            });
          });
        });
      });
      return selectorsToSync2;
    }
...
// <stdin>
var sectionChanged = new CustomEvent("quarto-sectionChanged", {
  detail: {},
  bubbles: true,
  cancelable: false,
  composed: false
...
window.document.addEventListener("DOMContentLoaded", function(_event) {
      ...
      return response.json().then(function(listingPaths) {
        ...
        for (const listingPath of listingPaths) {
          ...
          for (const item of listingPath.items) {
            ...
            if (encodedItem === currentPagePath || encodedItem === currentPagePath + "index.html") {
  ...
  const leftSideConflictEls = arrConflictingEls.filter((el) => {
    ...
    return Array.from(el.classList).find((className) => {
      return className !== "column-body" && className.startsWith("column-") && !className.endsWith("right") && !className.endsWith("container") && className !== "column-margin";
    });
  });
  ...
  window.quartoToggleReader = () => {
    const slowTransition = (slow) => {
      const manageTransition = (id, slow2) => {
